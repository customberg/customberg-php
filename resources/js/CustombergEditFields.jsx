...
window.CustombergEditFields = (block) => {
    return class extends Component {
                        ...
                        Object.keys(this.props.attributes[field.name]).map(subFieldName => {
                            ...
                            if (subField && subField.multilanguage) {
                                if (typeof this.props.attributes[field.name][subFieldName] == 'string') {
                                    this.props.attributes[fieldName][subFieldName] = { 'ro': this.props.attributes[fieldName][subFieldName] };
                                }
                            }
                        });
                    }
        ...
        render() {
            let isSelected = true;
            let isInnerSelected = true;
            // let isSelected = window.Laraberg.wordpress.data.select('core/block-editor').isBlockSelected(this.props.clientId);
            // let isInnerSelected = window.Laraberg.wordpress.data.select('core/block-editor').hasSelectedInnerBlock(this.props.clientId, true);
            return (
                        ...
                        <>
                            {block.multilanguage && (
                                <div style={styles.langButtonsContainer}>
                                    <button style={{ backgroundColor: this.props.attributes.activeLang == 'ro' ? '#1E90FF' : 'rgba(0,0,0,0.1)', marginRight: 10, borderRadius: 5, borderWidth: 0 }} type="button" onClick={() => this.changeLang({ name: 'ro' })}>
                                        <span style={{ color: this.props.attributes.activeLang == 'ro' ? '#fff' : '#000', padding: '5px 10px' }}>ro</span>
                                    </button>
                                    <button style={{ color: this.props.attributes.activeLang == 'en' ? '#fff' : '#000', backgroundColor: this.props.attributes.activeLang == 'en' ? '#1E90FF' : 'rgba(0,0,0,0.1)', marginRight: 10, borderRadius: 5, borderWidth: 0 }} type="button" onClick={() => this.changeLang({ name: 'en' })}>
                                        <span style={{ color: this.props.attributes.activeLang == 'en' ? '#fff' : '#000', padding: '5px 10px' }}>en</span>
                                    </button>
                                </div>
                            )}
                                    ...
                                    <div key={field.name + index} style={{ padding: 4 }}>
                                        {this.renderField(field, this.props.attributes[field.name], (value) => {
                                            if (field.multilanguage) {
                                                this.props.setAttributes({ [field.name]: { ...this.props.attributes[field.name], [this.props.attributes.activeLang]: value } });
                                            }
                                            else
                                                this.props.setAttributes({ [field.name]: value });
                                        })}
                                    </div>
                                ))}
                            </div>
        ...
        renderField(field, value, onChange) {
            ...
            if (field.type == 'blocks') {
                if (value && window.Laraberg.wordpress.data.select('core/block-editor').getBlock(this.props.clientId)?.innerBlocks.length == 0) {
                    window.Laraberg.wordpress.data.dispatch('core/block-editor').insertBlocks(window.Laraberg.wordpress.blocks.parse(value), 0, this.props.clientId);
                }
                return (
                    <label style={{ width: '100%' }}>
                        <div>{field.label}</div>
                        <div style={{ padding: 10, border: '1px solid #ccc' }}>
                            <InnerBlocks
                                ...
                                onChange={(content) => onChange(window.Laraberg.wordpress.blocks.serialize(content))}
            ...
            if (field.type == 'select') {
                let select_options = field.options ? (
                    Object.keys(field.options).map(itemKey => (
                        { value: itemKey, label: field.options[itemKey] }
                    ))
                ) : [];
            ...
            if (field.type == 'repeatable') {
                ...
                return (
                    <div>
                        ...
                        <div>
                            {value.map((item, index) => (
                                <div key={field.name + index} style={styles.repeatableItem}>
                                    <button type="button" onClick={() => deleteItem(index)} style={styles.repeatableDeleteBtn}>X</button>
                        ...
                        <div>
                            <button type="button" onClick={() => !reachedMaxItems && addItem()} disabled={reachedMaxItems}>
                                Add item
                            </button>
                        </div>
                    </div>
...
function ColorPickerField({ value, onChange }) {
    ...
    return (
        <>
            <div
                onClick={(event) => { event.preventDefault(); event.stopPropagation(); setVisible(true); }}
